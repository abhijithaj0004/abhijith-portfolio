...
class _NavbarState extends State<Navbar> {
  ...
  void _scrollTo(GlobalKey key) {
    setState(() => _menuOpen = false);
    // Wait a frame so the closing menu doesn't change the scroll
    // viewport's size mid-calculation, then scroll to the target section.
  ...
  Widget build(BuildContext context) {
    ...
    // A plain Column instead of a Stack+Positioned overlay: the dropdown
    // now grows the Navbar's real layout height via AnimatedSize, so its
    // items sit inside the actual hit-testable area instead of being
    // painted outside it (which made them visible but untappable).
    return Material(
